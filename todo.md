...
- [x] Narrator profile system — Aoede (primary), Charon (nighttime, not yet wired), Neural2-B (fallback)
- [x] SSML builder — 1.8s leading break, sentence-length-aware inter-sentence pauses, ellipsis pacing
- [x] Phoneme override system — 27 Singapore place names wrapped in IPA tags before sentence splitting
- [x] Audio upload — Supabase `whisper-audio` bucket, `city/{citySlug}/whispers/{whisperId}.mp3`
- [x] Approval trigger — `PATCH /:id/status` fires fire-and-forget TTS pipeline, `GenerationJob` tracks progress
- [x] Narrator preview endpoint — `POST /admin/whispers/preview-narration`, returns base64 MP3 + SSML
- [x] WhisperCard loading states (G-7) and audio preloading (G-8)
- [x] 22 SSML/phoneme tests green
- [x] E2E validated on-device — approve → audioUrl populated → Aoede plays → silences intentional → 85% write path regression-free
