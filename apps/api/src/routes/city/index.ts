...
export async function cityRoutes(app: FastifyInstance) {
  ...
  app.get<{
    ...
    // Use geohash neighbours to narrow DB query before haversine
    ...
    const pois = await prisma.poi.findMany({
      ...
      take: limit * 2, // overfetch before distance filter
    })
    ...
    const nearby = pois
      .map((poi) => {
        ...
        return {
          ...
          distanceMeters,
          visited: false, // resolved client-side from local store
        }
      })
